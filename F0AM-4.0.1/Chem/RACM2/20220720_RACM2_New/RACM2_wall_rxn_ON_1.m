...
'XYO'; 'XYP'; 'XYO2'; 'XYOP'; 'wO3'; 'wNO'; 'wHNO3'; 'wONIT'; 'wPPN'; 'wPAN';};
...
fAPIP(i)=fAPIP(i)-1; fNO(i)=fNO(i)-1; fHO2(i)=fHO2(i)+0.82; fNO2(i)=fNO2(i)+0.78; 
fHCHO(i)=fHCHO(i)+0.23; fALD(i)=fALD(i)+0.43; fACT(i)=fACT(i)+0.11; fKET(i)=fKET(i)+0.44; 
fORA1(i)=fORA1(i)+0.07; fONIT(i)=fONIT(i)+0.22; 
...
k(:,i) = 1.38E-6;
Gstr{i,1} = 'PAN';  
fPAN(i)=fPAN(i)-1; fwPAN(i)=fwPAN(i)+1;

i=i+1; %R374
Rnames{i} = 'PPN=wPPN';
k(:,i) = 1.31E-6;
Gstr{i,1} = 'PPN';  
fPPN(i)=fPPN(i)-1; fwPPN(i)=fwPPN(i)+1;
...
i=i+1; %R376
Rnames{i} = 'wONIT=HO2+0.2ALD+NO2+0.8KET';
k(:,i) = JONIT;
Gstr{i,1} = 'wONIT'; 
fwONIT(i)=fwONIT(i)-1; fHO2(i)=fHO2(i)+1; fALD(i)=fALD(i)+0.2; fNO2(i)=fNO2(i)+1; fKET(i)=fKET(i)+0.8; 

i=i+1; %R032
Rnames{i} = 'wPAN=ACO3+NO2';
k(:,i) = JPAN1;
Gstr{i,1} = 'wPAN'; 
fwPAN(i)=fwPAN(i)-1; fACO3(i)=fACO3(i)+1; fNO2(i)=fNO2(i)+1;

%mmarvin 20151020
i=i+1; %R033
Rnames{i} = 'wPAN=MO2+NO3+CO2';
k(:,i) = JPAN2;
Gstr{i,1} = 'wPAN'; 
fwPAN(i)=fwPAN(i)-1; fMO2(i)=fMO2(i)+1; fNO3(i)=fNO3(i)+1; fCO2(i)=fCO2(i)+1;

i=i+1; %R033
Rnames{i} = 'wPPN=RCO3+NO2';
k(:,i) = JPAN1;
Gstr{i,1} = 'wPPN'; 
fwPPN(i)=fwPPN(i)-1; fRCO3(i)=fRCO3(i)+1; fNO2(i)=fNO2(i)+1;

i=i+1; %R001
Rnames{i} = 'wO3=O3P+O2';
k(:,i) = JO3P;
Gstr{i,1} = 'wO3'; 
fwO3(i)=fwO3(i)-1; fO3P(i)=fO3P(i)+1;

i=i+1; %R002
Rnames{i} = 'wO3=O1D+O2';
k(:,i) = JO1D;
Gstr{i,1} = 'wO3'; 
fwO3(i)=fwO3(i)-1; fO1D(i)=fO1D(i)+1;

%herogeneous chemistry
i=i+1; %R377
Rnames{i} = 'N2O5=HNO3';
k(:,i) = 1.58E-4;
Gstr{i,1} = 'N2O5'; 
...
k(:,i) = 1.E-4;
Gstr{i,1} = 'ONIT'; 
fONIT(i)=fONIT(i)-1; fHNO3(i)=fHNO3(i)+1; 
